...
@pushOnce('scripts')
    ...
    <script type="module">
        app.component('v-login-desktop-dropdown', {
            template: '#v-login-desktop-dropdown-template',
            ...
            methods: {
                getLoginContent() {
                    this.$axios.get("{{ route('shop.api.home.login_desktop_dropdown') }}")
                        .then(response => {
                            this.$refs.container.innerHTML = response.data.data;
                        })
                        .catch(error => {
                            console.log(error);
                        });
